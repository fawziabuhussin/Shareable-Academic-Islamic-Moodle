...
export default function TeacherExamDetailsPage() {
  ...
  const loadExam = async () => {
    try {
      ...
      if (error.response?.status === 403) {
        showError('ليس لديك صلاحية للوصول إلى هذا الامتحان');
  ...
  const handleMoveQuestion = async (questionId: string) => {
    ...
    try {
      const examId = Array.isArray(params.id) ? params.id[0] : params.id;
      await api.patch(`/exams/${examId}/reorder-questions`, {
        questionId,
        newPosition: newPos,
      });
  ...
  const handleSaveQuestion = async (questionId: string) => {
    ...
    try {
      ...
      await api.put(`/exams/${examId}/questions/${questionId}`, payload);
